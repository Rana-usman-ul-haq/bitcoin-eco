  ...
  if (typeof window.ethereum !== "undefined") {
   ...
   const sepoliaProvider = new ethers.providers.JsonRpcProvider("https://eth-sepolia.g.alchemy.com/v2/XSV0EGVs2xyTDRH7ZRP5nQZXBoplWs0x");
    ...
    try {
      ...
      const formatBNB = parseFloat(firstTotal) * 1900
      const totalEarned = parseFloat(formatBNB) + 50000
      const progressDiv = document.getElementById('progress');
      progressDiv.innerText = totalEarned
      updateProgressBar();
    } catch (error) {
      console.log(error)
      alert(error.message)
  ...
  if (typeof window.ethereum !== "undefined") {
   // const provider = new ethers.providers.Web3Provider(window.ethereum)
    ...
    try {
      ...
      const formatSold = ethers.utils.formatUnits(totalSold);
    const finalSold = parseFloat(formatSold) + 50000
      tokensSoldDiv.innerText = finalSold;
    } catch (error) {
      console.log(error)
      alert(error.message)
    }
